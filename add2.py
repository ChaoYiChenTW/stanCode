...
Name: Elaine Chen
------------------------
TODO: add two given integers in ListNode data structure
"""
...
def add_2_numbers(l1: ListNode, l2: ListNode) -> ListNode:
    """
    :param l1: ListNode, input
    :param l2: ListNode, input
    :return l3: ListNode, the sum of l1 and l2
    """
    l3 = ListNode(0, None) 
    curl3 = l3
    curl1 = l1
    ...
    while curl1 or curl2:
        if not curl1:
            cursum = curl3.val + curl2.val
            curl2 = curl2.next
        elif not curl2:
            cursum = curl3.val + curl1.val
            curl1 = curl1.next
        else:
            cursum = curl1.val + curl2.val + curl3.val 
            curl1 = curl1.next
            curl2 = curl2.next
        curl3.val = divmod(cursum, 10)[1]
        curl3.next = ListNode(divmod(cursum, 10)[0], None)
        pre = curl3
        curl3 = curl3.next
    if curl3.val == 0:
        pre.next = None
